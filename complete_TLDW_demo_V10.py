...
chat_history = []
temp_str = ""
# Importing Streamlit for web application development.
import streamlit as st
# Importing Streamlit for web application development.
from streamlit_player import st_player
#from langchain.document_loaders import PyPDFLoader
# Importing langchain modules for NLP and chat functionalities.
...
# Importing os module for OS-level operations.
import os
#import PyPDF2
# Importing csv module for CSV file operations.
import csv
# Importing Streamlit for web application development.
import streamlit as st
import pandas as pd
...
# Using FAISS for similarity search and clustering of vectors.
...
# Setting up ChatOpenAI for conversational AI.
    ...
    with col1:
        ...
        with container_video:
            ...
            selected_keyword = st.selectbox("Select your prompt based on the keyword:", options=list(keyword_dict.keys()))
...
# Initializing a list to store chat history.
        ...
        please dont say: "based on the context", "as per the provided context", "unfortunately, i cannot asnwer the question", "i can not answer the question based on the provided audio/video trasncripts," "also provide the timestamps in answers in float/decimal format" while asnwering the questions, 
        try asnwering the questions as much as possible \n\n  now, please asnwer this question if prossible provide video timestamps (in seconds interger format and also it's converted format into the hours:minutes:seconds fomrat in brackets, i.e. 150 seconds [00:03:00], 240 seconds[00:04:00], remember to only give video timestamps in integer ( not float/decimal format) format followed byit's converted representation in hours:minutes:seconds fomrat in [] brackets from where this content/asnwer is refered from 
        (the timestamps in the context is in seconds and you provide the asnwer in seconds format only folowed by the seconds format converted into [hours:minutes:seconds] format) if relevant cite your asnwer to a video timestamp whenever possible even when it is not explicitly asked in the question,  
        please asnwer the question intutively and never say "based on the context", "as per the provided context", "unfortunately, i connot asnwer the question", "i can not answer the question based on the provided audio/video trasncripts" or other similar terms while asnwering the questions aslways try to provide some kind of answer. please remember the timestamps you refer in the answer should only be in seconds integer format followed by it's converted representation into hours:minutes:seconds format (i.e. 150 seconds [00:02:30] etc.) \n QUESTION : """
...
# Initializing a list to store chat history.
        st.button("clear chat history", on_click=clear_chat_history, args=(video['chat_history'],))
